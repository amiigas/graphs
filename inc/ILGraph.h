//
// Created by Aleksy on 2020-04-12.
// Incidence List Graph
//

#ifndef GRAPHS_ILGRAPH_H
#define GRAPHS_ILGRAPH_H
...
public:
    ...
    ILGraph();
    ILGraph(int noOfVerts, double density);
    ...
    friend std::ostream& operator << (std::ostream& stream, const ILGraph &graph) {
...
ILGraph::ILGraph(int noOfVerts, double density) {
...
void ILGraph::loadFromFile(const std::string& filename) {
...
List<Vertex>& ILGraph::getVerts() {
    return verts;
}

List<Edge>& ILGraph::getEdges() {
    return edges;
}

Vertex* ILGraph::getStartingVert() {
    return startingVertex;
}

Vertex& ILGraph::getVert(int i) {
    return verts[i];
}

Edge& ILGraph::getEdge(int i) {
    return edges[i];
}

List<Edge*>& ILGraph::getAdjacentTo(int i) {
    return verts[i].getList();
};

void ILGraph::addVertex(int element) {
    verts.push_front(Vertex(verts.getSize(), element));
}

void ILGraph::addEdge(int weight, Vertex& start, Vertex& end) {
    Edge e(weight,start,end);
    auto p = edges.push_front(e);
    start.getList().push_front(p);
    end.getList().push_front(p);
}

bool ILGraph::areConnected(Vertex& v1, Vertex& v2) {

    if (!v1.getList().isEmpty()) {
        for (int i = 0; i < v1.getList().getSize(); i++) {
            if (v1.getList()[i]->getStartVert() == &v2 ||
                v1.getList()[i]->getEndVert() == &v2)
                return true;
        }
    }
    return false;
